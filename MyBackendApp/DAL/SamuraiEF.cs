...
namespace MyBackendApp.DAL
{
    public class SamuraiEF 
    {
        ...
        public void DeleteSamuraiBattle(int samuraiId, int BattelId)
        {
            try
            {
                var battlewithsamurai = _dbcontext.Battles.Include(b => b.Samurais.Where(s => s.Id == samuraiId)).FirstOrDefault(s => s.BattleId == BattelId);
        ...
        {
            var samurais = _dbcontext.Samurais.FromSqlInterpolated($"exec dbo.SamuraisWhoSaidAWord {text}").ToList();
            return samurais;
        }

        public IEnumerable<Samurai> GettAllWithQuery()
        {
            var samurai = _dbcontext.Samurais.FromSqlRaw("select * from Samurais").ToList();
            return samurai;
        }
